...
// initialize Kafka
let allTopics = ["user-service-topic", "role-service-topic"];
let topicsToSubscribe = ["user-service-topic", "role-service-topic"];
  ...
  .then(async () => {
    await Kafka.suscribe(
      ...
      (topic: any, partition: any, message: any) => {
        ...
        } else if (topic == "role-service-topic") {
          const { action, data } = JSON.parse(message?.value?.toString());
          RoleFacade.consumer(action, data);
        }
      }
    );
